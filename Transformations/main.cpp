...
void test_scale() {
	std::vector<Vec4f> cube_vertices{
		{ -1.0f, -1.0f, 1.0f, 1.0f },
		{ -1.0f, -1.0f, -1.0f, 1.0f },
		{ 1.0f, -1.0f, -1.0f, 1.0f },
		{ 1.0f, -1.0f, 1.0f, 1.0f },
	...
	Mat4f scale_x = Mat4f::create_scale_x(2.0f);
	Mat4f scale_y = Mat4f::create_scale_y(0.5f);
	Mat4f scale_z = Mat4f::create_scale_z(1.5f);
	Mat4f scale_total = scale_z * scale_y * scale_x;

	for (const Vec4f& v : cube_vertices) {
		Vec3f vertex_after = scale_total * v;
		std::cout << std::format("[{}, {}, {}]\n", vertex_after.x, vertex_after.y, vertex_after.z);
	}
}
